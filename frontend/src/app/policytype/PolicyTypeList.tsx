...
import { PolicyTypeDetails } from "./PolicyTypeDetails";

const ROUTE = "policy-type-list";

const POLICY_TYPES = gql(/* GraphQL */ `
  query policyTypes {
    policyTypes {
      description
      id
...
const DELETE__POLICY_TYPE = gql(/* GraphQL */ `
  mutation delete_PolicyType($id: Long!) {
    delete_PolicyType(id: $id)
  }
`);

export const PolicyTypeList = observer(
  ({ onSelect }: EntityListScreenProps) => {
    ...
    const { loading, error, data } = useQuery(POLICY_TYPES);

    const [executeDeleteMutation] = useMutation(DELETE__POLICY_TYPE);
      ...
      (id?: string) => {
        const params: OpenInBreadcrumbParams = {
          breadcrumbCaption: intl.formatMessage({
            id: "screen.PolicyTypeDetails"
          }),
          component: PolicyTypeDetails
        };
        if (id != null) {
          params.props = { id };
        }
...
function getUpdateFn(e: any) {
  return (cache: ApolloCache<any>) => {
    cache.modify({
      fields: {
        policyTypes(existingRefs, { readField }) {
