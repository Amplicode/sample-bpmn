...
const DELETE__POLICYHOLDER = gql(/* GraphQL */ `
    mutation delete_Policyholder($id: Long!) {
        delete_Policyholder(id: $id)
    }
`);

export const PolicyholderList = observer(({onSelect}: EntityListScreenProps) => {
    const screens: Screens = useScreens();
    const intl = useIntl();
    const match = useRouteMatch<{ entityId: string }>(`/${ROUTE}/:entityId`);
    const history = useHistory();

    const {loading, error, data} = useQuery<{ policyholders: Policyholder[] }>(POLICYHOLDERS);

    const [executeDeleteMutation] = useMutation(DELETE__POLICYHOLDER);
    ...
    const openEditor = useCallback(
        (id?: string) => {
            const params: OpenInBreadcrumbParams = {
                breadcrumbCaption: intl.formatMessage({id: "screen.PolicyholderDetails"}),
                component: PolicyholderDetails
            };
            if (id != null) {
                params.props = {id};
            }
    ...
    const items = data?.policyholders;
...
function getUpdateFn(e: any) {
    return (cache: ApolloCache<any>) => {
        cache.modify({
            fields: {
                policyholders(existingRefs, {readField}) {
